...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut sender_map: HashMap<String,tokio::sync::mpsc::Sender<v1::Resource>> = HashMap::new();

    let (virtual_network_sender, virtual_network_receiver): (tokio::sync::mpsc::Sender<v1::Resource>, tokio::sync::mpsc::Receiver<v1::Resource>) = tokio::sync::mpsc::channel(1);
...
async fn subscribe(client: &mut ConfigControllerClient<Channel>, sender_map: &mut HashMap<String,tokio::sync::mpsc::Sender<v1::Resource>>) -> Result<(), Box<dyn Error>> {
    ...
    while let Some(resource) = stream.message().await? {
        ...
        if let Some(sender) = sender_map.get(resource.kind.as_str()) {
            ...
            sender.send(resource.clone()).await;
        }
    }
    Ok(())
}
